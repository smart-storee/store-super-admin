...
export default function StoresPage() {
  ...
  }, [debouncedSearchTerm, filterStatus, stores]);

  const handleDeleteStore = (storeId: number, storeName: string) => {
    setConfirmDelete({ storeId, storeName });
  };
  ...
  const confirmDeleteStore = async () => {
    ...
    try {
      setDeletingStoreId(confirmDelete.storeId);
      const response = await apiRequest<any>(
        `/api/v1/super-admin/stores/${confirmDelete.storeId}`,
        {
          method: "DELETE",
        }
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-5 py-2.5 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-all font-semibold"
        ...
        <div className="flex gap-4 items-center">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500"
            />
          </div>
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100"
          >
                      ...
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
                          store.billing_status === "active"
                            ...
                            : "bg-gray-100 dark:bg-slate-800/30 text-gray-800 dark:text-slate-400"
                        }`}
                      >
                        {store.billing_status
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">
                      <div className="flex gap-2 flex-wrap">
                        ...
                        {store.products_enabled && (
                          <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400">
                            Products
                          </span>
                        )}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-800/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          title="Delete store"
                        >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-card-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-slate-200 dark:border-slate-700 sticky top-0 bg-white dark:bg-slate-800">
              ...
              <form onSubmit={handleCreateStore} className="p-6 space-y-5">
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                  ...
                  <button
                    ...
                    className="flex-1 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-4 py-3 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-all font-semibold"
                  >
                    Cancel
                  </button>
                </div>
      ...
      <ConfirmModal
        ...
        title="Delete Store"
        message={`Are you sure you want to delete "${confirmDelete?.storeName}"? This will deactivate the store and all associated users. This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
        variant="danger"
